...
static int load_index_internal(_Index *index) {
    index->count = 0;
    ...
    while (fscanf(f, "%o %64s %llu %u %511s", &mode, hex, &mtime, &size, path) == 5) {
        if (index->count >= MAX_INDEX_ENTRIES) break;
        _IndexEntry *e = &index->entries[index->count];
        e->mode = mode;
...
static int write_tree_level(_IndexEntry *entries, int count, const char *prefix, ObjectID *id_out) {
    Tree tree;
    tree.count = 0;
...
int tree_from_index(ObjectID *id_out) {
    _Index index;
    if (load_index_internal(&index) != 0) return -1;
